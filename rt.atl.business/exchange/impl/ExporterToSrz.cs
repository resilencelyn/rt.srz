// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ExporterToSrz.cs" company="Rintech">
...
namespace rt.atl.business.exchange.impl
{
  ...
  public class ExporterToSrz : ExchangeBase
  {
        ...
        try
        {
          using (var sessionAtl = sessionFactorySrz.OpenSession())
          {
            ...
            try
            {
              ...
              var queryOver = session.QueryOver(() => statement)
                .Where(x => x.Status.Id == StatusStatement.New || x.Status.Id == StatusStatement.Performed || x.Status.Id == StatusStatement.Declined)
              ...
              queryOver = session.QueryOver(() => statement)
                 .Where(x => x.Status.Id == StatusStatement.New || x.Status.Id == StatusStatement.Performed || x.Status.Id == StatusStatement.Declined)
    ...
    private void ExportByQuery(IQueryOver<StatementMaximum, StatementMaximum> query, ISession sessionAtl, Przlog log, IJobExecutionContext context)
    {
      var session = sessionFactoryPvp.GetCurrentSession();
      ...
      {
        var list = query.OrderBy(x => x.DateFiling).Asc
                  .ThenBy(x => x.Id).Asc
                  .Skip(i * 5000)
                  .Take(5000)
    ...
    {
      var flag = true;
      var errorStringList = !string.IsNullOrEmpty(przBuff.Sflk) ? przBuff.Sflk.Split(',').Select(x => x.PadLeft(3, '0')) : new[] { "Not" };
      foreach (var error in errors.Where(x => errorStringList.Any(y => y == x.Code)).Select(procedure => new Error
                {
                  Code = procedure.Code,
                  Message1 = RecodingErros.Recoding(int.Parse(procedure.Code), procedure.Caption.ToLower().ToUpperFirstChar()),
                  Statement = st,
                  Repl = przBuff.Repl
                }))
      {
      ...
      {
        session.Save(
          new Error { Code = "-1", Message1 = przBuff.Repl ?? "СРЗ не вернула ни одной ошибки, но даные не приняла.", Statement = st, Repl = przBuff.Repl });
      }
    ...
    private void ExportByList(ISession session, ISession sessionAtl, Przlog log, IList<StatementMaximum> query, IJobExecutionContext context)
    {
      if (query.Count == 0)
      {
        return;
    ...
    private void ExportStatement(StatementMaximum st, Przlog log, ISession sessionAtl, ISession session)
    {
      ...
      var tempCertificate = st.MedicalInsurances.Where(x => x.IsActive).FirstOrDefault(x => x.PolisType.Id == PolisType.В);

      //Экспорт временного свидетельства
      if (!st.IsExportTemp && tempCertificate != null)
      {
        Przbuf przbuffTemp = WritePrzBuff(st, log, sessionAtl, tempCertificate, GetTypeOperationTemp(st));
      ...
      //Поиск полиса
      var polisCertificate = st.MedicalInsurances.Where(x => x.IsActive).SingleOrDefault(x => x.PolisType.Id != PolisType.В);
      ...
      string op)
    {
      ...
      var przBuff = new Przbuf
                      {
                        ...
                        Rnname = address.Area,
                        City = address.City,
                        Np = !string.IsNullOrEmpty(address.Town) ? address.Town : !string.IsNullOrEmpty(address.City) ? address.City : address.Subject,
                        Ul = address.Street,
                        Dom = address.House,
                        Kor = address.Housing,
                        Kv =
                        ...
                        Pcity = address2.City,
                        Pnp = !string.IsNullOrEmpty(address2.Town) ? address2.Town : !string.IsNullOrEmpty(address2.City) ? address2.City : address2.Subject,
                        Pul = address2.Street,
                        Pdom = address2.House,
      ...
      if (st.PreviousStatement != null)
      {
        ...
        // Документ УДЛ
        if (st.PreviousStatement.ResidencyDocument != null && st.PreviousStatement.ResidencyDocument.DocumentType != null)
        {
      ...
      przBuff.Zaddr = string.Format("{0}{1}",
        address.IsNotStructureAddress.HasValue != null ? Convert.ToInt16(address.IsNotStructureAddress.Value).ToString() : "0",
        address2.IsNotStructureAddress.HasValue != null ? Convert.ToInt16(address2.IsNotStructureAddress.Value).ToString() : "0");
