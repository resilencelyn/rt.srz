// --------------------------------------------------------------------------------------------------------------------
// <copyright file="WorkstationManager.cs" company="Rintech">
...
namespace rt.srz.business.manager
{
  ...
  public partial class WorkstationManager
  {
    ...
    /// The entity.
    /// </param>
    public override void Delete(model.srz.Workstation entity)
    {
    ...
    public byte[] GetCertificateKey(string workstationName, string pdpCode, int version, int type)
    {
      ...
      {
        var workstation =
          session.QueryOver<Workstation>().Where(
            x => x.Name == workstationName && x.PointDistributionPolicy.Id == level2.Id).List().FirstOrDefault();
        ...
        if (workstation != null)
        {
          sert =
            sertificateUecManager.GetBy(
              x => x.IsActive && x.Version == version && x.Workstation.Id == workstation.Id && x.Type.Id == type).
              FirstOrDefault();
          if (sert != null)
          {
            return sert.Key;
        ...
        if (level2.Parent != null)
        {
          sert =
            sertificateUecManager.GetBy(
              x => x.IsActive && x.Version == version && x.Smo.Id == level2.Parent.Id && x.Type.Id == type).
              FirstOrDefault();
          return sert != null ? sert.Key : null;
        }
      }
    ...
    public int GetCurrentCryptographyType(string workstationName, string pdpCode)
    {
      ...
      var workstation = session.QueryOver<Workstation>()
    ...
    public string GetCurrentReaderName(string worstationName, string pdpCode)
    {
      ...
      var workstation = session.QueryOver<Workstation>()
        .JoinAlias(x => x.PointDistributionPolicy, () => pdp)
        .Where(x => x.Name == worstationName && pdp.Code == pdpCode).List()
        .FirstOrDefault();
      return workstation != null && !string.IsNullOrEmpty(workstation.UecReaderName)
               ? workstation.UecReaderName
               : string.Empty;
    ...
    public WorkstationSettingParameter[] GetWorkstationSettings(string workstationName, string pdpCode)
    {
      ...
      Organisation pdp = null;
      var settings = session.QueryOver<Setting>()
    ...
    public void SaveWorkstationSettings(string workstationName, string pdpCode, WorkstationSettingParameter[] settingsArr)
    {
      ...
      try
      {
        foreach (var dtoSetting in settingsArr)
        {
          ...
          var setting =
            session.QueryOver<Setting>().Where(
              x => x.Name == dtoSetting.Name && x.Workstation != null && x.Workstation.Id == workstation.Id).List().
              FirstOrDefault();
          if (setting == null)
          {
