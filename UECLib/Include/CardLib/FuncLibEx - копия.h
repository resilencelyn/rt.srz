...
/* Description
   ...
   Осуществляет первоначальные проверки и настройки:
     ...
     * Инициализирует сведения о применении приложения и
       согласовывает тип используемого картой криптоалгоритма ГОСТ
       или RSA.
     * Инициализирует флаги поддержки механизма электронной
       подписи, поддержки картой "длинных данных" и команды MSE.
     * Проверяет наличие каталога секторов и инициализирует
       список размещённых на карте секторов прикладных данных.
     * Проверяет наличие сертификата открытого ключа терминала и
       ОКО.
   ...
   IL_RETCODE - Код ошибки.
   Summary
   Выбор приложения карты УЭК, первоначальные проврки и
   настройки.                                                      */
IL_FUNC IL_RETCODE flAppSelect(IL_CARD_HANDLE* phCrd, IL_BYTE *out_pData, IL_WORD *out_pDataLen);

/* Description
   Выполняет повторный выбор приложения карты УЭК без проверок и
   возврата информации.<p />
   Все инициализированные при первоначальном выборе
   приложения, настройки остаются в силе.
   See Also
   flAppSelect
   Parameters
   phCard :  Указатель на описатель карты.
   ...
   IL_RETCODE - Код ошибки.
   Summary
   Повторный выбор приложения карты.                             */
IL_FUNC IL_RETCODE flAppReselect(IL_CARD_HANDLE* phCrd);

/* Description
   Выполняет процедуру аутентификации терминала.<p />
   Данная процедура предназначена для того, чтобы
   идентификационное приложение убедилось, что терминал обладает
   закрытым ключом, который соответствует открытому ключу,
   указанному в сертификате открытого ключа терминала.<p />
   В процессе аутентификации терминала:
     * Производится проверка сертификатов терминала.
     * Выполняется процедура динамической аутентификации с
       использованием согласованного ранее криптоалгоритма ГОСТ или
       RSA.
     * Устанавливается защищённая сессия для обмена сообщениями
       между терминалом и картой.
   See Also
   flAppSelect
   Parameters
   phCard :  Указатель на описатель карты.
...
/* Description
   Формирует запрос на аутентификацию ИД-приложения для оказания
   услуги.
   See Also
   opApiPrepareAppAuthRequest
   Parameters
   phCard :              Указатель на описатель карты.
   ifOnline :            Признак формирования запроса для
                         аутентификации в режиме Online.
   in_pTermAuthData :    Указатель на буфер c входными
                         TLV\-данными терминала\:
                         * Идентификатор услуги '9F15'.
                         * Случайное число терминала '9F37'.
                         * Штамп времени выполнения операции по
                           часам терминала '9F21'.
                         * Дополнительные сведения об операции
                           '9F03'.
                         * Хэш&#45;значение запроса на оказание
                           услуги 'DF02'.
                         * Сведения о терминале '9F1C'.
                         * Сжатое значение открытого ключа
                           терминала '9F1D' (для карт версии 1.1).
                         * Сведения об идентификационном
                           приложении 'C2' (для карт версии 1.1).
                         * Номер версии идентификационного
                           приложения '9F08' (для карт версии 1.1).
                         * Открытый ключ терминала '9F1E' (для
                           карт версии 1.1).
   TermAuthDataLen :     Длина входных TLV\-данных терминала.
   out_pAuthData :       Указатель на буфер для формируемого
                         запроса.
   inout_pAuthDataLen :  Указатель на максимальный размер буфера
                         и фактическую длину сформированного
                         запроса.
   Returns
   IL_WORD - Код ошибки.
   Summary
   Формирование запроса на аутентификацию ИД-приложения для
   оказания услуги.                                                 */
...
/* Description
   Формирует запрос на аутентификацию ИД-приложения для
   установки защищённой сессии с эмитентом.
   See Also
   ...
   in_pTermAuthData :    Указатель на буфер c входными
                         ...
                         * Штамп времени выполнения операции по
                           часам терминала '9F21'.
                         * Дополнительные сведения об операции
                           '9F03'. Для операции 'Разблокировка КРП'
                           инициализируется ПИН&#45;блоком для
                           временного значения КРП, формируемого
                           терминалом. Иначе инициализируется
                           нулевыми значениями.
                         * Хэш&#45;значение запроса на оказание
                           услуги 'DF02' инициализируется нулевыми
                           значениями.
                         * Сведения о терминале '9F1C'.
                         * Сжатое значение открытого ключа
                           терминала '9F1D' (для карт версии 1.1).
...
/* Description
   ...
   Фраза контрольного приветствия отображается перед вводом
   держателем карты ПИН. По данной фразе держатель убеждается,
   что аутентификация терминала выполнена успешно, терминал
   является доверенным, работа с ПИН и персональными данными
   будет безопасной.
   See Also
   Parameters
   phCard :           Указатель на описатель карты.
   out_pPassPhrase :  Указатель на выходной буфер для строки с
                      фразой контрольного приветствия. Считанная
                      с карты строка автоматически конвертируется
                      в Win\-1251. При отсутствии на карте
                      механизма контрольного приветствия
...
/* Description
   ...
   будет безопасной.
   See Also
   opCtxSetPassPhrase opCtxGetPassPhrase
   Parameters
   phCard :          Указатель на описатель карты.
   in_pPassPhrase :  Указатель на входной буфер cо строкой фразы
                     контрольного приветствия в кодировке
                     Win\-1251. Записываемая на карту строка
...
/* Description
   ...
   Для карт версии 1.0 сведения о терминале считываются из
   секции 'TerminalInfo' конфигурационного файла 'terminal.ini'.<p />
   Для карт версии 1.0 сведения о терминале извлекаются из
   сертификата открытого ключа терминала.
   See Also
   Parameters
   phCard :        Указатель на описатель карты.
   out_pData :     Указатель на буфер для возвращаемых данных.
...
/* Description
   ...
   See Also
   opCtxSetDigitalSignatureBuf opApiMakeDigitalSignature
   Parameters
   phCard :               Указатель на описатель карты.
   in_pAuthRequest :      Указатель на сформированный ранее
                          запрос на аутентификацию ИД\-приложения
                          для оказания услуги.
   AuthRequestLen :       Длина запроса на аутентификацию
                          ИД\-приложения.
   out_pSign :            Указатель на буфер для возвращаемой
                          электронной подписи.
   inout_pSignLen :       Указатель на размер буфера электронной
                          подписи и длину возвращаемых данных.
   out_pCertChain :       Указатель на цепочку сертификатов ключа
                          проверки электронной подписи.
   inout_pCertChainLen :  Указатель на максимальный размер буфера
                          и длину возвращаемой цепочки
                          сертификатов.
   Returns
   IL_RETCODE - Код ошибки.
   Summary
